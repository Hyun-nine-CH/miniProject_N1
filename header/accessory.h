...
private:
    ...
    int price;
    int stock;
    // Accessory 클래스는 악세서리의 정보를 저장하는 클래스이며 멤버 변수를 private로 선언하여 외부에서 직접 접근하지 못하도록 함
    // Accessory는 Amp, Tuner, PowerSupply, Cable와 달리 purpose 멤버 변수가 존재함
    // purpose는 악세서리의 용도를 나타내며, Amp, Tuner, PowerSupply, Cable와 달리 purpose 멤버 변수의 이름이 다름
    // 따라서 Accessory는 Amp, Tuner, PowerSupply, Cable과 달리 상속받는 요소가 없다. 따라서, 이렇게 멤버 변수를 직접 선언
