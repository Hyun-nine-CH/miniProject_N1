...
// 이 클래스는 공통 제품 속성을 캡슐화한 추상화 기반 클래스(Base Class) 역할을 하며, Amp, Tuner, Accessory 등에서 상속받는 구조입니다.
...
// SubProduct 생성자, 멤버 변수를 초기화한다.
// SubProduct는 Amp, Tuner, Accessory 등에서 상속받아 특정 제품군의 특성만 분리하여 관리하는 파생 클래스입니다.
// SubProduct는 Amp, Tuner, Accessory의 부모 클래스이며, Amp, Tuner, Accessory는 SubProduct를 상속받는다.
// std::string 타입을 const 참조로 받음으로써 불필요한 복사를 방지하고 있습니다.
...
// const 함수로 선언되어 있어, 멤버를 수정하지 않는다는 것을 명시합니다.
...
// 직접 멤버 변수에 접근하지 않고 메서드를 통해 조작하는 방식은 캡슐화 원칙을 지킵니다.